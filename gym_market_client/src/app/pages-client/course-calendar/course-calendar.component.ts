...
@Component({
	selector: 'app-client-course-calendar',
	standalone: true,
...
export class CourseCalendarComponent implements OnInit {
	...
	from = this.toInputDate(this.shiftDate(-7));
	to = this.toInputDate(this.shiftDate(75));
	isLoading = false;
	...
	load(): void {
		...
		this.liveSessionService
			.getMyCalendar(this.fromDate, this.toDate)
			...
			.subscribe({
				...
				error: () => {
					this.isLoading = false;
					this.toastService.show('Failed to load calendar', 'error');
					this.cdr.markForCheck();
				},
			});
	}
	...
	get nextItem(): CourseCalendarItem | null {
		const now = Date.now();
		return this.items.find(item => new Date(item.startsAt).getTime() >= now) ?? null;
	}
